...
describe("auditTarget", () => {
  ...
  it("reports gaps for the bad directory fixture", async () => {
    ...
    expect(report.gaps.some((gap) => gap.id === "agents-md" && gap.fix.includes("AGENTS.md"))).toBe(
      true,
    );
  });
  ...
  it("flags a server.json that uses the retired schema URL", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        description: "Example",
        repository: { url: "https://example.com", source: "github" },
        version: "1.0.0",
        packages: [],
      }),
    );
    ...
    expect(check(report, "server-json").note).toContain(
  ...
  it("warns when an offer uses pressure copy", async () => {
    ...
    await writeFile(
      path.join(directory, "offer.jsonld"),
      '{"@type":"Offer","name":"Access","price":"10"}',
    );
    const report = await auditTarget(directory);
  ...
  it("audits a URL with an injected fetch implementation", async () => {
    ...
    const server = generate("server-json", {
      name: "Example product",
      description: "An example agent product",
    ...
    const orgJsonLd =
      '{"@context":"https://schema.org","@type":"Organization","name":"Example","contactPoint":{"@type":"ContactPoint","email":"support@example.com","contactType":"customer support"},"address":{"@type":"PostalAddress","addressCountry":"US"}}';
    const homeText =
      "Example product helps agents create, schedule, and publish content across every major platform from one API. ".repeat(
        8,
      );
    ...
    const fetcher = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      ...
      if (url === `${base}/agents.md`)
        return new Response(agents, { status: 200, headers: { "content-type": "text/markdown" } });
      if (url === `${base}/server.json`)
      ...
      if (url === base && /text\/markdown/.test(accept))
        return new Response(markdown, {
          status: 200,
          headers: { "content-type": "text/markdown", vary: "Accept, Accept-Encoding" },
        });
      if (url === base)
      ...
      if (url === `${base}/.well-known/agent.json`)
        return new Response('{"name":"Example"}', {
          status: 200,
          headers: { "content-type": "application/json" },
        });
      if (url === `${base}/.well-known/agent-card.json`)
        return new Response(`{"name":"Example","url":"${base}"}`, {
          status: 200,
          headers: { "content-type": "application/json" },
        });
      if (url === `${base}/.well-known/api-catalog`)
        return new Response(
          '{"linkset":[{"anchor":"/api","service-desc":[{"href":"/openapi.json"}]}]}',
          { status: 200, headers: { "content-type": "application/linkset+json" } },
        );
      if (url === `${base}/auth.md`)
      ...
      if (url === `${base}/agents`)
        return new Response("<html>agents</html>", {
          status: 200,
          headers: { "content-type": "text/html" },
        });
      if (url === `${base}/.well-known/ai-plugin.json`)
        return new Response(
          '{"schema_version":"v1","name_for_model":"example","api":{"type":"openapi","url":"/openapi.json"},"auth":{"type":"none"}}',
          { status: 200, headers: { "content-type": "application/json" } },
        );
      // External registries (round 3):
      if (url.startsWith("https://www.wikidata.org/w/api.php"))
        return new Response('{"query":{"searchinfo":{"totalhits":1},"search":[{"title":"Q1"}]}}', {
          status: 200,
          headers: { "content-type": "application/json" },
        });
      if (url.startsWith("https://registry.npmjs.org/-/v1/search"))
        return new Response(
          '{"objects":[{"package":{"name":"example-product","links":{"homepage":"https://example.com"}}}]}',
          { status: 200, headers: { "content-type": "application/json" } },
        );
      if (url.startsWith("https://registry.modelcontextprotocol.io/v0/servers"))
        return new Response(
          '{"servers":[{"server":{"name":"io.example/example","websiteUrl":"https://example.com"}}]}',
          { status: 200, headers: { "content-type": "application/json" } },
        );
  ...
  it("detects external discovery gaps when registries have no match", async () => {
    ...
    const fetcher = async (input: RequestInfo | URL): Promise<Response> => {
      ...
      if (url === base)
        return new Response(
          "<html><head><title>NoListings</title></head><body><h1>x</h1></body></html>",
          { status: 200, headers: { "content-type": "text/html" } },
        );
      // registries return empty results
      if (url.startsWith("https://www.wikidata.org/w/api.php"))
        return new Response('{"query":{"searchinfo":{"totalhits":0},"search":[]}}', {
          status: 200,
        });
      if (url.startsWith("https://registry.npmjs.org/-/v1/search"))
        return new Response('{"objects":[]}', { status: 200 });
      if (url.startsWith("https://registry.modelcontextprotocol.io/v0/servers"))
        return new Response('{"servers":[]}', { status: 200 });
      return new Response("", { status: 404 });
    };
    const report = await auditTarget(base, { fetch: fetcher });
  ...
  it("rejects an empty ai-plugin.json as not a usable manifest", async () => {
    ...
    const fetcher = async (input: RequestInfo | URL): Promise<Response> => {
      ...
      if (url === base)
        return new Response(
          "<html><head><title>EmptyPlugin</title></head><body><h1>x</h1></body></html>",
          { status: 200, headers: { "content-type": "text/html" } },
        );
      if (url === `${base}/.well-known/ai-plugin.json`)
  ...
  it("rejects an ai-plugin.json that identifies itself but has no api/auth block", async () => {
    ...
    const fetcher = async (input: RequestInfo | URL): Promise<Response> => {
      ...
      if (url === base)
        return new Response(
          "<html><head><title>Bare</title></head><body><h1>x</h1></body></html>",
          { status: 200, headers: { "content-type": "text/html" } },
        );
      if (url === `${base}/.well-known/ai-plugin.json`)
        return new Response('{"schema_version":"v1","name_for_model":"example"}', {
          status: 200,
          headers: { "content-type": "application/json" },
        });
      return new Response("", { status: 404 });
  ...
  it("treats a registry error as inconclusive rather than a confirmed gap", async () => {
    ...
    const fetcher = async (input: RequestInfo | URL): Promise<Response> => {
      ...
      if (url === base)
        return new Response(
          "<html><head><title>RateLimited</title></head><body><h1>x</h1></body></html>",
          { status: 200, headers: { "content-type": "text/html" } },
        );
  ...
  it("does not false-positive an npm/mcp match on a generic host label", async () => {
    ...
    const fetcher = async (input: RequestInfo | URL): Promise<Response> => {
      const url = String(input);
      if (url === base)
        return new Response("<html><head><title>App</title></head><body><h1>x</h1></body></html>", {
          status: 200,
          headers: { "content-type": "text/html" },
        });
      ...
      if (url.startsWith("https://registry.npmjs.org/-/v1/search"))
        return new Response(
          '{"objects":[{"package":{"name":"unrelated-application-toolkit","links":{"homepage":"https://unrelated.example"}}}]}',
          { status: 200 },
        );
      if (url.startsWith("https://registry.modelcontextprotocol.io/v0/servers"))
        return new Response(
          '{"servers":[{"server":{"name":"io.unrelated/mapping-tool","websiteUrl":"https://unrelated.example"}}]}',
          { status: 200 },
        );
      if (url.startsWith("https://www.wikidata.org/w/api.php"))
        return new Response('{"query":{"searchinfo":{"totalhits":0},"search":[]}}', {
  ...
  it("does not pass content-without-js or org-schema off a broken homepage", async () => {
    ...
    const fetcher = async (input: RequestInfo | URL): Promise<Response> => {
      const url = String(input);
      if (url === base)
        return new Response(richErrorPage, {
          status: 500,
          headers: { "content-type": "text/html" },
        });
      return new Response("", { status: 404 });
  ...
  it("does not count an h1 hidden inside a script as server-rendered content", async () => {
    ...
    const text =
      "Loading placeholder text that is not real page content and should not count. ".repeat(8);
  ...
  it("does not pass crawler-reachable when the agent User-Agent hits a block page", async () => {
    ...
    const fetcher = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      ...
      if (url === base && /ora-agent/.test(userAgent)) {
        return new Response("<html><body>Access Denied - are you a robot?</body></html>", {
          status: 200,
          headers: { "content-type": "text/html" },
        });
      }
  ...
  it("does not count h1 and text hidden inside an HTML comment as server-rendered content", async () => {
    ...
    const hiddenText =
      "This text is commented out and never rendered by a browser or crawler. ".repeat(8);
  ...
  it("does not pass org-schema when contactPoint and address are present but null", async () => {
    ...
    const html =
      '<html><head><script type="application/ld+json">{"@type":"Organization","contactPoint":null,"address":null}</script></head><body></body></html>';
    const fetcher = async (input: RequestInfo | URL): Promise<Response> => {
      const url = String(input);
      if (url === base)
        return new Response(html, { status: 200, headers: { "content-type": "text/html" } });
  ...
  it("warns a table-stakes URL that lacks the world-class signals", async () => {
    ...
    const server = generate("server-json", {
      name: "Plain",
      description: "A plain agent product",
      repository: "https://github.com/example/plain",
      npmPackage: "plain",
      version: "1.0.0",
    });
    const fetcher = async (input: RequestInfo | URL): Promise<Response> => {
      ...
      if (url === base)
        return new Response(
          '<script type="application/ld+json">{"@type":"Offer","price":"0"}</script>',
          { status: 200, headers: { "content-type": "text/html" } },
        );
    ...
    const report = await auditTarget(base, { fetch: fetcher });
    expect(report.score).toBeLessThan(100);
    expect(report.gaps.map((g) => g.id)).toEqual(
      expect.arrayContaining(["well-known-agent", "openapi", "agents-page"]),
    );
  });
});
...
describe("generators", () => {
  ...
  it("generates both static and Next route guidance", () => {
    ...
    expect(generate("agents-route", { ...params, framework: "next" })).toContain(
...
it("keeps the reference directory out of package discovery", async () => {
  await expect(readFile(path.join(fixtures, "good", "AGENTS.md"), "utf8")).resolves.toContain(
    "## Offer",
  );
});
