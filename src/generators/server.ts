...
export function generateServerJson(config: AigentifyConfig): string {
  const serverName = config.name.includes("/")
    ? config.name
    : `io.github.pooriaarab/${config.name.toLowerCase().replace(/[^a-z0-9._-]+/g, "-")}`;
    ...
    server.packages = [
      {
        ...
        identifier: config.npmPackage ?? "aigentify",
        version: config.version ?? "0.1.0",
        transport: { type: "stdio" },
        runtimeHint: "npx",
        packageArguments: [{ type: "positional", valueHint: "subcommand", value: "mcp" }],
      },
    ];
