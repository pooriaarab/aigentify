...
function wikidataHit(d: unknown): boolean {
  ...
  const hits =
    (d as { query?: { searchinfo?: { totalhits?: number } } })?.query?.searchinfo?.totalhits ?? 0;
  return hits > 0;
}
...
function npmHomepageMatches(
  o: { package?: { links?: { homepage?: string } } },
  host: string,
): boolean {
...
async function probeNpm(
  ...
  const d = await fetchJson(
    fetcher,
    `https://registry.npmjs.org/-/v1/search?size=20&text=${encodeURIComponent(npmQuery)}`,
  );
  ...
  const objects = Array.isArray(rawObjects)
    ? (rawObjects as { package?: { name?: string; links?: { homepage?: string } } }[])
    : [];
...
function mcpServerRecord(entry: unknown): Record<string, unknown> {
  return ((entry as { server?: Record<string, unknown> })?.server ?? entry) as Record<
    string,
    unknown
  >;
}
...
function mcpHostMatches(server: Record<string, unknown>, host: string): boolean {
  ...
  const repoHost = safeHost(
    String((server?.repository as { url?: string } | undefined)?.url ?? ""),
  );
...
async function probeMcpRegistry(
  ...
  const results = await Promise.all(
    [slug, host].map((q) =>
      fetchJson(
        fetcher,
        `https://registry.modelcontextprotocol.io/v0/servers?search=${encodeURIComponent(q)}`,
      ),
    ),
  );
