...
export function createMcpServer(): McpServer {
  ...
  }, async ({ target }) => ({ content: [{ type: 'text', text: JSON.stringify(await auditTarget(target), null, 2) }] }));
  server.registerTool('gen', {
    description: 'Generate an agent-native artifact as text.',
    inputSchema: { artifact: z.enum(artifacts), params: z.record(z.string(), z.unknown()).optional() },
  }, async ({ artifact, params }) => ({ content: [{ type: 'text', text: generate(artifact as GeneratedArtifact, (params ?? {}) as GenerateParams) }] }));
  return server;
}
