...
program
  .name(PKG_NAME)
  .version(VERSION)
  .description("Audit and generate agent-native product surfaces.");

program
  .command("audit [target]")
  .description("Audit a local directory or live URL.")
  .option("--json", "print JSON instead of the report")
  .action(async (target = ".", options: { json?: boolean }) => {
...
program
  .command("init")
  .description("Write starter AGENTS.md and server.json into the current directory.")
...
program
  ...
  .description("Generate an agent-native artifact.")
  .option("-o, --out <path>", "write to a file instead of stdout")
  .option("--name <name>")
  .option("--description <description>")
  .option("--repository <url>")
  .option("--mcp-url <url>")
  .option("--npm-package <name>")
  .option("--api-base-url <url>")
  .option("--signup-url <url>")
  .option("--price <price>")
  .option("--price-currency <currency>")
  .option("--framework <framework>", "route framework, such as static or next", "static")
  .action(async (artifact: string, options: Record<string, string | undefined>) => {
    const offer =
      options.price === undefined && options.priceCurrency === undefined
        ? undefined
        : { price: options.price, priceCurrency: options.priceCurrency };
