...
export async function loadAigentifyConfig(
  directory = process.cwd(),
): Promise<AigentifyConfig | undefined> {
  const file = await findConfigFile(directory);
  if (!file) return undefined;
  const jiti = createJiti(import.meta.url);
  const module = (await jiti.import(file)) as {
    default?: AigentifyConfig;
    aigentifyConfig?: AigentifyConfig;
  };
  const config = module.aigentifyConfig ?? module.default;
  if (!config)
    throw new Error(`${path.basename(file)} must export a default config or aigentifyConfig.`);
